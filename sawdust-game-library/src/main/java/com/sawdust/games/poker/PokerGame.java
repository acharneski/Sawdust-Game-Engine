...
public abstract class PokerGame extends MultiPlayerCardGame
{
    ...
    {
        Null, Bidding, Drawing, Showing, Complete
    }
    ...
    private static final Logger LOG = Logger.getLogger(PokerGame.class
            .getName());
    ...
    {
        final PropertyConfig propertyConfig = config.getProperties().get(
                GameConfig.NUM_PLAYERS);
        if (null != propertyConfig)
            return propertyConfig.getInteger();
        return 2;
    }
    ...
    private final HashMap<Participant, PlayerState> _playerStates = new HashMap<Participant, PlayerState>()
    {
        ...
        {
            System.out.println(String.format("_playerStates set: %s = %s", key
    ...
    private void declareWinner(final GameSession gameSession)
            throws GameException
    {
        ...
        for (int i = 0; i < NUMBER_OF_PLAYERS; i++)
        {
            ...
            if (_playerStates.get(playerName) == PlayerState.Complete)
            {
                ...
                final PokerHand playerHand = PokerHandPattern
                        .FindHighest(playerCards);
                this.doAddMessage("%s has a %s", getDisplayName(playerName),
                        playerHand.getName());
        ...
        this.doAddMessage("<strong>%s won with %s</strong>",
                getDisplayName(winner), winningHand.getName());
        ...
        if (winner instanceof Player)
        {
            ...
            String type = "Win/Go";
            String event = String.format("I won a game of Poker with a !",
                    winningHand.getName());
        ...
        for (int i = 0; i < NUMBER_OF_PLAYERS; i++)
        {
            ...
            {
                String type = "Lose/Go";
                String event = String.format("I lost a game of Poker to %s!",
                        displayName);
                ((Player) otherPlayer).doLogActivity(new ActivityEvent(type,
                        event));
            }
        }
    }

    public void doBet(final Participant player, final int bet)
            throws GameException
    {
        /*
         * R-P-0001 In casino play the first betting round begins with the
         * player to the left R-P-0002 of the big blind, and subsequent rounds
         * begin with the player to the dealer's R-P-0003 left. Home games
         * typically use an ante; the first betting round begins R-P-0004 with
         * the player to the dealer's left, and the second round begins with
         * R-P-0005 the player who opened the first round. In this version, the
         * running bet is started at the level of the game ante and betting goes
         * from Player 1 onward. Each player can raise, see the current bet
         * level, or fold.
         */
        if (getCurrentPhase() != GamePhase.Bidding)
            throw new GameLogicException("Invalid state: " + getCurrentPhase());
        if (bet < _roundBet)
            throw new GameLogicException(String.format(
                    "Cannot bet %d, current bet is %d", bet, _roundBet));
        if (bet < _roundBet)
            throw new GameLogicException(String.format(
                    "Cannot bet %d, current bet is %d", bet, _roundBet));
        if (!getPlayerManager().isCurrentPlayer(player))
            throw new GameLogicException("It is not your turn");
        if (_playerStates.get(player) != PlayerState.Bidding)
            throw new GameLogicException("Invalid player state");
        ...
        if (payAmount > 0)
        {
            if (player instanceof Player)
            {
                ...
                {
                    this.doAddMessage(MessageType.Compact, "(All in: %d) ",
                            balance);
        ...
        if (raiseAmount > 0)
        {
            ...
            this.doAddMessage(MessageType.Compact, "%s raises the bet to %d",
                    getDisplayName(player), bet);
            this.doAddMessage("");
            _roundBet = bet;
        } else
        {
            this.doAddMessage(MessageType.Compact, "%s calls",
        ...
        {
            this
                    .doAddMessage("<strong>Betting is complete. Please draw new cards.</strong>");
            setCurrentPhase(GamePhase.Drawing);
        } else
        {
            this.doAddMessage(MessageType.Compact, "%s's turn: ",
                    getDisplayName(nextPlayer));
        }
    }
    ...
    {
        /*
         * R-P-0011 If more than one player remains after the first round, the
         * "draw" phase R-P-0012 begins. Each player specifies how many of their
         * cards they wish to replace R-P-0013 and discards them. The deck is
         * retrieved, and each player is dealt in R-P-0014 turn from the deck
         * the same number of cards they discarded so that each R-P-0015 player
         * again has five cards. R-P-0016 A second "after the draw" betting
         * round occurs beginning with the player R-P-0017 to the dealer's left
         * or else beginning with the player who opened the R-P-0018 first round
         * (the latter is common when antes are used instead of blinds).
         * R-P-0019 This is followed by a showdown if more than one player
         * remains, in which R-P-0020 the player with the best hand wins the
         * pot. There is no second-round betting before the showdown
         */
        ...
        {
            IndexCard t = (IndexCard) getToken(new IndexPosition(playerIndex,
                    cardSlot));
            if (null == t)
            {
                numberOfCards++;
        ...
        this.doAddMessage("Player %s draws %d cards", getDisplayName(player),
                numberOfCards);
    ...
    public void doFold(final Participant user) throws GameException
    {
        if (getCurrentPhase() != GamePhase.Bidding)
            throw new GameLogicException("Invalid state: " + getCurrentPhase());
        if (!getPlayerManager().isCurrentPlayer(user))
            throw new GameLogicException("It is not your turn");
        ...
        if (isBidding)
            throw new GameLogicException("Invalid player state: " + playerState);
        _playerStates.put(user, PlayerState.Folded);
        this.doAddMessage(MessageType.Compact, "%s folds.",
                getDisplayName(user));
        this.doAddMessage("");
        Participant nextPlayer = getPlayerManager().gotoNextPlayer();
    ...
    public void dropCards(final Participant email,
            final ArrayList<Integer> cardIndex) throws GameException
    {
        /*
         * R-P-0011 If more than one player remains after the first round, the
         * "draw" phase R-P-0012 begins. Each player specifies how many of their
        ...
        for (final Integer cardSlot : cardIndex)
        {
            ...
            if (null == card)
            {
                this.doAddMessage("Warning: No card found at index %d",
                        cardSlot);
                continue;
            }
            doRemoveToken(card);
    ...
    {
        final List<AgentFactory<? extends Agent<?>>> agentFactories = super
                .getAgentFactories();
            ...
            throws GameException
    {
        ...
        if (GamePhase.Drawing == _currentPhase)
        {
            ...
            {
                final IndexCard t = (IndexCard) getToken(new IndexPosition(
                        playerIndex, cardNumber));
                if (null != t)
                {
                    l = new GameLabel("DISCARD " + cardNumber,
                            new IndexPosition(CURVE_CARD_BUTTONS, cardNumber),
                            "Discard");
            ...
            labelNumber++;
            l = new GameLabel("STAY", new IndexPosition(CURVE_CMD_BUTTONS,
                    labelNumber++), "Draw Cards");
        ...
        } else if (GamePhase.Bidding == _currentPhase)
        {
            ...
            l = new GameLabel("POOL", new IndexPosition(CURVE_CMD_BUTTONS,
                    labelNumber++), String.format("Current Wager: %d",
                    _roundBet));
            returnValue.add(l);
            ...
            l = new GameLabel("SEE", new IndexPosition(CURVE_CMD_BUTTONS,
                    labelNumber++), String.format("Call", requiredBet));
            l.setCommand("Call");
            returnValue.add(l);
            ...
            {
                l = new GameLabel("RAISE1", new IndexPosition(
                        CURVE_CMD_BUTTONS, labelNumber++), String.format(
                        "Raise %d", ante, ante + _roundBet - currentBet));
                l.setCommand("Raise " + ante);
                returnValue.add(l);
                ante *= 5;
                if (ante < maxRaise)
                {
                    l = new GameLabel("RAISE5", new IndexPosition(
                            CURVE_CMD_BUTTONS, labelNumber++), String.format(
                            "Raise %d", ante));
                    l.setCommand("Raise " + ante);
                    returnValue.add(l);
                ...
                {
                    l = new GameLabel("ALL IN", new IndexPosition(
                            CURVE_CMD_BUTTONS, labelNumber++), "Go All In");
                    l.setCommand("Go All In");
                    returnValue.add(l);
                }
            }

            l = new GameLabel("FOLD", new IndexPosition(CURVE_CMD_BUTTONS,
                    labelNumber++), "Fold");
            l.setCommand("Fold");
            returnValue.add(l);
        } else if (GamePhase.Complete == _currentPhase)
        {
            ...
            l = new GameLabel("DEAL", new IndexPosition(CURVE_CMD_BUTTONS,
                    labelNumber++), String.format("Deal Again"));
            l.setCommand("Deal");
            returnValue.add(l);
    ...
    {
        if (GamePhase.Null == _currentPhase)
            return 15;
        return super.getUpdateTime();
    }

    @Override
    public ArrayList<GameCommand<?>> getMoves(final Participant access)
            throws GameException
    {
            ...
            for (int cardNumber = 0; cardNumber < NUMBER_OF_CARDS; cardNumber++)
            {
                ...
                if (null != t)
                {
                    ...
                    returnValue.add(new GameCommand()
                    {
                        ...
                        @Override
                        public CommandResult doCommand(Participant p,
                                String commandText) throws GameException
                        {
                            com.sawdust.games.poker.Commands.Drop_Cards
                                    .doCommand((Player) p, getSession(),
                                            cardNumberStr);
            ...
            }
            {
                returnValue.add(new GameCommand()
                {
                    ...
                    public CommandResult doCommand(Participant p,
                            String commandText) throws GameException
                    {
                        com.sawdust.games.poker.Commands.Draw_Cards.doCommand(
                                (Player) p, getSession(), "");
                        return new CommandResult<PokerGame>(PokerGame.this);
                    }
                });
        ...
        } else if (GamePhase.Bidding == _currentPhase)
        {
            ...
            final int requiredBet = _roundBet - currentBet;
            final int balance = (access instanceof Player) ? ((Player) access)
                    .getAccount().getBalance() : -1;
            ...
            if (_roundBet > 0)
            {
                ...
                returnValue.add(new GameCommand()
                {
                    ...
                    public CommandResult doCommand(Participant p,
                            String commandText) throws GameException
                    {
                        com.sawdust.games.poker.Commands.Bet.doCommand(
                                (Player) p, getSession(), betAmt);
                        return new CommandResult<PokerGame>(PokerGame.this);
                    }
                });
            }
            ...
            {
                {
                    final String betAmt = Integer
                            .toString(_roundBet + maxRaise);
                    returnValue.add(new GameCommand()
                    {
                        ...
                        public CommandResult doCommand(Participant p,
                                String commandText) throws GameException
                        {
                            com.sawdust.games.poker.Commands.Bet.doCommand(p,
                                    getSession(), betAmt);
                            return new CommandResult<PokerGame>(PokerGame.this);
                        }
                    });
...

            {
                returnValue.add(new GameCommand()
                {
                    ...
                    public CommandResult doCommand(Participant p,
                            String commandText) throws GameException
                    {
                        com.sawdust.games.poker.Commands.Fold.doCommand(
                                (Player) p, getSession(), "");
                        return new CommandResult<PokerGame>(PokerGame.this);
                    }
                });
    ...
    {
        final ArrayList<GameLabel> returnValue = new ArrayList<GameLabel>();
        for (int playerIndex = 0; playerIndex < getPlayerManager()
                .getPlayerCount(); playerIndex++)
        {
            final Participant player = getPlayerManager().getPlayerName(
                    playerIndex);
            returnValue.add(new GameLabel("PlayerLabel " + playerIndex,
                    new IndexPosition(POS_PLAYER_LABEL, playerIndex),
                    getDisplayName(player)));
        }
        return returnValue;
    }
            ...
            throws GameException
    {
        ...
        if (curveIndex == findPlayer)
            return _mainHand.getPositionN(key.getCardIndex());
        else if ((curveIndex >= 0) && (curveIndex < NUMBER_OF_PLAYERS))
        {
            ...
            // modulus
            // INCORRECTLY
            final Position positionN = _playerHands.get(relativePlayer)
                    .getPositionN(key.getCardIndex());
            positionN.setZ(key.getCardIndex());
            return positionN;
        } else if (curveIndex == CURVE_CARD_BUTTONS)
            return _mainHand.getPositionN(key.getCardIndex()).add(
                    OFFSET_CARD_BUTTON);
        else if (curveIndex == CURVE_CMD_BUTTONS)
            return POSITION_COMMAND.add(OFFSET_COMMAND
                    .scale(key.getCardIndex()));
        else if (curveIndex == POS_PLAYER_LABEL)
        {
            int relativePlayer = (key.getCardIndex() - findPlayer)
                    % NUMBER_OF_PLAYERS;
            relativePlayer--;
            if (relativePlayer < 0)
            {
            ...
            // modulus
            // INCORRECTLY
            if (findPlayer == key.getCardIndex())
                return _mainHand.getPositionN(0).add(OFFSET_PLAYERLABEL);
            else
                return _playerHands.get(relativePlayer).getPositionN(0).add(
                        OFFSET_PLAYERLABEL);
        } else
    ...
    protected void initPositions(final int playerCount, final int cardCount)
    {
        ...
        {
            _playerHands.add(new ParametricLine(currentPosition,
                    currentPosition.add(_handVector), cardCount));
            currentPosition = currentPosition.add(OFFSET_OPPONENTS);
        }
        _mainHand = new ParametricLine(POSITION_PLAYER, POSITION_PLAYER
                .add(OFFSET_PLAYER_CARD.scale(cardCount)), cardCount);
    }
    ...
    {
        if (!getPlayerManager().isFull())
            return false;
        if (getCurrentPhase().equals(
                com.sawdust.games.poker.PokerGame.GamePhase.Complete))
            return false;
        if (getCurrentPhase().equals(
                com.sawdust.games.poker.PokerGame.GamePhase.Null))
            return false;
        return true;
    }
    ...
    {
        LOG.info(String.format("Game state changed from %s to %s",
                _currentPhase.toString(), currentPhase.toString()));
    ...
    public GameState doStart() throws GameException
    {
        ...
        if (isntNull && isntComplete)
            throw new GameLogicException("Invalid state: " + getCurrentPhase());
        doClearTokens();
        getDeck().setReshuffleEnabled(true);
        for (int player = 0; player < NUMBER_OF_PLAYERS; player++)
        {
            final Participant thisPlayer = getPlayerManager().getPlayerName(
                    player);
            this.doAddMessage(MessageType.Compact, "%s's hand: ",
                    getDisplayName(thisPlayer)).setTo(thisPlayer.getId());
            for (int cardSlot = 0; cardSlot < NUMBER_OF_CARDS; cardSlot++)
            {
                final IndexCard t = doDealNewCard(new IndexPosition(player,
                        cardSlot));
                t.setOwner(thisPlayer);
                t.setPrivate("VR");
                t.setMovable(true);
                this.doAddMessage(MessageType.Compact, "(%s) ", t.getCard())
                        .setTo(thisPlayer.getId());
            }
            this.doAddMessage("");
        }
        _playerStates.clear();
    ...
    @Override
    public PokerGame moveAgents() throws GameException
    {
        super.moveAgents();
        if (getCurrentPhase() == GamePhase.Drawing)
        {
            ...
            {
                if ((_playerStates.get(p) == PlayerState.Ready)
                        && (p instanceof Agent<?>))
                {
