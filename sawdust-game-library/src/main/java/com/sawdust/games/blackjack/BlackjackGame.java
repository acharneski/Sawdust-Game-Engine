...
public abstract class BlackjackGame extends IndexCardGame
{
    ...
    @Override
    public BlackjackGame doAddPlayer(final Participant s) throws GameException
    {
    ...
    BlackjackGame doEndTurn() throws GameException
    {
        ...
        /*
         * R-B-0049 The payoff for a player blackjack is 3:2, meaning that the casino pays R-B-0050 $3 for each $2
         * originally bet. (There are many single-deck games which R-B-0051 pay only 6:5 for a blackjack.) Blackjack
         * pays the same as all wins, at 1:1.
         */
        if (dealerBust)
        {
    ...
    public BlackjackGame doHit(int i) throws GameException
    {
        final GameSession gameSession = getSession();
        if (GamePhases.Playing != _currentPhase) throw new GameLogicException("Game is not in progress");
        final int totalScore = doHitUtil(i);
        this.doAddMessage("Player has %d", totalScore);
        if (totalScore > 21)
        {
            doLose(i, gameSession);
        }
        return this;
    }

    @Deprecated
    int doHitUtil(final int playerIndex)
    {
        final int cardNumber = getCardCount(playerIndex);
        final IndexCard newCard = doDealNewCard(new IndexPosition(playerIndex, cardNumber));
    ...
    BlackjackGame doLose(int i, GameSession gameSession) throws GameException
    {
    ...
    @Override
    public BlackjackGame doStart() throws GameException
    {
        ...
        /*
         * R-B-0047 The minimum and maximum bets are posted on the table. The payoff on most R-B-0048 bets is 1:1,
         * meaning that the player wins the same amount as he bets. All bets are fixed at the ante level set at the game
         * beginning.
         */
        if (null != _owner)
        {
        ...
        /*
         * R-B-0001 In casino blackjack, the dealer faces one to seven players from behind R-B-0002 a kidney-shaped
         * table. Each player plays his hand independently against R-B-0003 the dealer. At the beginning of each round,
         * the player places a bet in R-B-0004 the "betting box" and receives an initial hand of two cards. The object
         */

        /*
         * R-B-0020 Cards are dealt in three ways, either from one or two hand-held decks, R-B-0021 from a box
         * containing four to eight decks called a "shoe," or from a shuffling R-B-0022 machine. When dealt by hand, the
         * player's two initial cards are face-down, R-B-0023 while the dealer has one face-up card called the "upcard"
         * and one face-down R-B-0024 card called the "hole card." (In European blackjack, the dealer's hole R-B-0025
         * card is not actually dealt until the players all play their hands.) When R-B-0026 dealt from a shoe, all
         * player cards are normally dealt face-up, with minor R-B-0027 exceptions. It shouldn't matter to the player
         * whether his cards are dealt R-B-0028 face-down or face-up since the dealer must play according to
         * predetermined R-B-0029 rules. If the dealer has less than 17, he must hit. If the dealer has
         */
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            for (int cardNumber = 0; cardNumber < 2; cardNumber++)
            {
                ...
                {
                    newCard = doDealNewCard(cardPosition);
                    card = newCard.getCard();
                    if (HACK_FORCE_PAIRS)
                    {
    ...
    public BlackjackGame doStay() throws GameException
    {
        if (GamePhases.Playing != _currentPhase) throw new GameLogicException("Game is not in progress");
        ...
        /*
         * R-B-0029 rules. If the dealer has less than 17, he must hit. If the dealer has R-B-0030 17 or more, he must
         * stand (take no more cards), unless it is a "soft 17" R-B-0031 (a hand that includes an ace valued as "11,"
         * for example a hand consisting R-B-0032 of Ace+6, or Ace+2+4). With a soft 17, the dealer follows the casino
         * rules R-B-0033 printed on the blackjack table, either to "hit soft 17" or to "stand on R-B-0034 all 17's."
         */
        int dealerScore = getScore(HAND_DEALER);
        this.doSaveState();
        ...
        while (dealerScore < 17)
        {
            ...
            dealerScore = doHitUtil(HAND_DEALER);
            this.doSaveState();
            this.doAdvanceTime(DEALER_PAUSE_MS);
        }
    ...
    int getCardCount(final int playerIndex)
    {
    ...
    public ArrayList<GameCommand<?>> getMoves(final Participant access) throws GameException
    {
         ...
         * "Stand," "Double Down," or "Split a pair." Each option requires R-B-0057 the use of a hand signal. At some
    ...
    int getScore(final int playerIndex)
    {
        int totalScore = 0;
        int numberOfAces = 0;
        int numberOfCards = 0;
    ...
    public BlackjackGame setCurrentPhase(GamePhases _currentPhase)
    {
