...
public class User implements Serializable
{
    ...
    private static String createNewGuest(final HttpServletResponse response, final String ip, final String agent)
    {
        ...
        final com.sawdust.engine.controller.entities.Account account = Account.Load(id);
    ...
    private static com.sawdust.gae.logic.User getUser_Cookied(final HttpServletRequest request, com.sawdust.gae.logic.User user)
    {
        final String email = getCookieLogin(request);
        if (null != email)
        {
        	LOG.fine("Cookie Auth: " + email);
            user = new com.sawdust.gae.logic.User(UserTypes.Member, email, "/logout.jsp");
        }
        return user;
    }

    private static com.sawdust.gae.logic.User getUser_Facebook(final HttpServletRequest request, com.sawdust.gae.logic.User user)
    {
        final String email = FacebookUser.getFacebookId(request);
        if (null != email)
        {
            user = new com.sawdust.gae.logic.User(UserTypes.Member, email, null);
    ...
    private static com.sawdust.gae.logic.User getUser_Google(final HttpServletRequest request, final AccessToken accessData,
            com.sawdust.gae.logic.User user)
    {
        ...
        if (null != email)
        {
            if (userService.isUserAdmin())
            {
                ...
                if ((null != userOverride) && !userOverride.isEmpty())
                {
                    ...
                    user = new com.sawdust.gae.logic.User(UserTypes.Admin, userOverride, null);
                }
                else
                {
                    user = new com.sawdust.gae.logic.User(UserTypes.Admin, email, userService.createLogoutURL("/"));
                }
            }
            else
            {
                LOG.fine(String.format("User is google-authenticated: %s", email));
                user = new com.sawdust.gae.logic.User(UserTypes.Member, email, userService.createLogoutURL("/"));
            }
        }
        return user;
    }
